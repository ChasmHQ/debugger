...
A call to Foundry's VM or console address may produce a `host_call` event with `address`,
`caller`, `data`, and `gas_limit`. The client must handle the calldata and send exactly one
`respond_host` request. `output` becomes the call's return data. Setting `revert` to `true`
returns that data with the EVM `REVERT` status. This keeps cheatcode policy outside the
engine and lets headless clients add host functions without changing REVM execution.
